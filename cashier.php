 ...
 <script src="js/script.js"></script>
	...
	include 'db.php';
		...
		if(isset($_POST['calcu'])){
			...
			$discountedAMT = $pri*$quan - $disc;

			$blank = "";

			$sql = "INSERT INTO pos2(item_name, price,qty, discount_amt, discounted_amt,total_qty, total_disc,total_discd) VALUES('$item_cal', '$pri', '$quan', '$disc', '$discountedAMT', '$quan', '$disc', '$discountedAMT')";
		...
		if(isset($_POST['sukliB'])){
			...
			$urSukli = getSukli($cash_sukli, $sukliAm);

			$query = "SELECT MAX(`pos2_id`) FROM `pos2`";
        	$result =$conn->query($query);

       	 	$queryresult = $result->fetch_array()['MAX(`pos2_id`)'] ?? '';

        	$insertCashChange = "UPDATE `pos2` SET cash_given = '$cash_sukli', cus_change = '$urSukli' WHERE `pos2_id` = '$queryresult'";
		...
		if(isset($_POST['new'])){
			if (isset($_POST['itemName'])) {
    			$itemposb = $_POST['itemName'];
			}
			if (isset($_POST['priceCash'])) {
    			$priposb = $_POST['priceCash'];
			}
			...
			$discountedAMT = $priposb*$currQty - $currD;

			$sql = "INSERT INTO pos2(item_name, price,qty, discount_amt, discounted_amt,total_qty, total_disc,total_discd) VALUES('$itemposb', '$priposb', '$currQty', '$currD', '$discountedAMT', '$currQty', '$currD',  '$discountedAMT')";
...
<body>
	...
	<div class="sideL">
		...
		<input type="text" id="changeCash" name="changeCash"value="<?php if (isset($urSukli)) echo $urSukli ?>" style="width: 250px; height: 100px">
		<br><br>
	</form>
		<div class="butDiv">
